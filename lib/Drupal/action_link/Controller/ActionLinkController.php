...
class ActionLinkController {
  ...
  function normal($action_link_plugin_style_id, $config_entity_type, $config_id, $entity_type, $entity_id, $new_state) {
    ...
    //$action_link_plugin = $config_entity->getLinkControllerPlugin();
    
    // Find out if this route is valid.
    $action_link_plugin->actionIsValid();
    
    // Find out if use has access.
    ...
    // Change state.
    $action_link_plugin->changeState($entity_id, 'entity!', array());
